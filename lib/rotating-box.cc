...
RotatingBox::RotatingBox(rgb_matrix::FrameCanvas *offscreen_canvas)
    : name{std::string("Rotating Box")}, cent_x(offscreen_canvas->width() / 2),
      ...
      rotate_square(
          std::min(offscreen_canvas->width(), offscreen_canvas->height()) *
          1.41),
      ...
      max_display(cent_x + display_square / 2), rotation(0) {
  return;
}
  ...
  for (int x = this->min_rotate; x < this->max_rotate; ++x) {
    for (int y = this->min_rotate; y < this->max_rotate; ++y) {
      ...
      if (x >= this->min_display && x < this->max_display &&
          y >= this->min_display &&
          y < this->max_display) { // within display square
        offscreen_canvas->SetPixel(
            rot_x + this->cent_x, rot_y + this->cent_y,
